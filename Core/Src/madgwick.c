...
static float calc_3D_norm(const AxisDataScaled_t *vector){
	float norm = 1.0f;
...
static float calc_beta_ratio(const AxisDataScaled_t *accels, float dt){
...
static AxisDataScaled_t calc_expected_accel_vector(const quat_t *q){
	AxisDataScaled_t expected_accel;
...
static AxisDataScaled_t calc_accel_vector_error(const AxisDataScaled_t *current_accel, const AxisDataScaled_t *expected_accel){
	AxisDataScaled_t error;
...
static AxisDataScaled_t angle_speed_correction(const quat_t *q, const AxisDataScaled_t *gyro, const AxisDataScaled_t *accel_error, float beta_ratio){
	AxisDataScaled_t corrected;
...
static quat_t calc_d_quat(const quat_t *q, const AxisDataScaled_t *angle_speed, float dt){
	quat_t dq;
...
void madgwick_run(MadgwickFilter *filter, const AxisDataScaled_t *accel, const AxisDataScaled_t *gyro){
	...
	AxisDataScaled_t gyro_corrected;
	...
	AxisDataScaled_t expected = calc_expected_accel_vector(&filter->orientation);

	// Ошибка между ожидаемым и реальным ускорением

	AxisDataScaled_t error = calc_accel_vector_error(accel, &expected);

	// Корреция угловой скорости на основе ошибки и бета коэффициента

	AxisDataScaled_t corrected_angle_speed = angle_speed_correction(&filter->orientation, &gyro_corrected, &error, beta);
